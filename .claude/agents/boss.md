...
## 工作流
1. 询问用户是否要删除缓存文件`clean_temp`，不允许直接读入任何文件；
2. 询问(a)用户顶层模块名、(b)包含输入RTL文件的filelist文件路径、(c)SPEC 文件路径（若有），不会直接读取任何输入文件。
3. **拆分阶段**：
...
6. **综合阶段**：
   ...
   - 将报告路径记录到状态文件。
7. 向用户汇总所有输出文件路径和最终结果。

## 注意事项
- 每个阶段完成后，必须将关键输出写入项目目录下的 `.claude/state.json`。
- 若任何阶段失败，非交互模式下自动停止；交互模式下询问用户是否继续。
- 综合阶段暂时没有搭建完成，先bypass；（TODO）

## 约束
- boss agent不允许直接读入文件，只做subagent之间的交互用
